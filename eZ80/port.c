...
static uint8_t port_read(uint16_t address, uint8_t loc, bool peek) {
    if ((address < 0x80) && (enadisaport == false)) { return control_read(address, peek); }else{ return ez80memaccess(address, 0, 3); }
...
static void port_write(uint16_t address, uint8_t loc, uint8_t value, bool peek) {
    ...
    if ((address < 0x80) && (enadisaport == false)){ control_write(address, value, peek); } else { ez80memaccess(address, value, 2); }
}
...
__declspec(dllexport) port_swi_specialport(bool tmpppp) { enadisaport = tmpppp; };
